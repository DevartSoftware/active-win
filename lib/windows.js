...
const voidPointer = ref.refType('pointer');
...
const version = ffi.Library('version', {
...
const readWstring = function (buf, offset, charCount) {
	return wchar.toString(buf.slice(offset, offset + charCount * wchar.size));//1
}

function wstring(str) {
	return ref.allocCString(str + '\0', 'ucs2');
}

function getVarEntry(mem) {
	const val = ref.alloc(voidPointer);
	const size = ref.alloc('uint32', 1);
	const res = version.VerQueryValueW(mem, wstring(String.raw`\\VarFileInfo\\Translation`), val, size);
	if (res) {
		const num = ref.readPointer(val, 0, 4);
		return num.readUInt16LE(0) << 16 | num.readUInt16LE(2);
	}
	return 0x040904E4;
}
...
function getFileVersionString(mem, name) {
	const val = ref.alloc(voidPointer);
	const size = ref.alloc('uint32', 0);
	const res = version.VerQueryValueW(mem, wstring(name), val, size);
	let data;
	if (res) {
		const buf = ref.readPointer(val, 0, size.deref() * wchar.size);//1
		data = readWstring(buf, 0, size.deref());
	}
	return data;
}

function getVersionInfoForCodePage(mem, codePage) {
	return getFileVersionString(mem, String.raw`StringFileInfo\\${codePage}\\FileDescription`);
}
...
function windows() {
	...
	// Get the file version info size, the size required by the `GetFileVersionInfoW` 
	...
	if (infoSize != 0) {
		...
		const mem = new Buffer.alloc(infoSize);
		if (version.GetFileVersionInfoW(processFileNameBufferClean, 0, infoSize, mem)) {

			//Get file description from the FileVersionInfo struct 
			const langId = getVarEntry(mem);
			const fileDescription = getVersionInfoForCodePage(mem, hexString(langId));
			if (fileDescription) {
	...
	return {
		...
		memoryUsage: getProcessMemoryInfoResult === 0 ? 0 : memoryCounters.WorkingSetSize
	};
